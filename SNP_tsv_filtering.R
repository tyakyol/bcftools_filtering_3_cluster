...
snps = snps[snps$X.7.results.sorted_Gifu_R.bam.GT %in% c('0/0', '0/1'), ]
# MQ >= 40
snps = snps[snps$X.5.MQ >= 40, ]
# DP >= 200
snps = snps[snps$X.4.DP >= 200, ]
# HOB <= 0.05
snps = snps[snps$X.6.HOB <= 0.05, ]
# Missing < 50%
missing = apply(snps[, 7:167], 1, function(x) sum(x == './.'))
snps = snps[missing <= 81, ]
# Must be biallelic
biallelic = apply(snps[, 7:167], 1, function(x) names(table(x)))
